			...
			<header className="p-8 text-white" style={{ backgroundColor: accentColor }}>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm ">
					...
					{data.personal_info?.Link && (
						<a target="_blank" href={data.personal_info?.Link} className="flex items-center gap-2">
							<Link className="size-4" />
							<span className="break-all text-xs">{data.personal_info.Link.split("https://www.")[1] ? data.personal_info.Link.split("https://www.")[1] : data.personal_info.Link}</span>
						</a>
					)}
